...
const Home = () => {
  ...
  const handleDailySessionsFilter = (category) =>
    setDailySessionsCategory(category);
  const handleActiveBreaksFilter = (category) =>
    setActiveBreaksCategory(category);
  const handleDailyExercisesFilter = (category) =>
    setDailyExercisesCategory(category);

  useEffect(() => {
    getUserFromCollection(user.id).then((response) => {
